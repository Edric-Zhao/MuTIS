...
def parse_args():
    ...
    parser.add_argument("--output_dir", default="./outputs_final", type=str)
...
def infer(args):
    ...
    # Define different max_tokens values to experiment with
    max_token_values = [ 200, 400,  600,  800,  1000,  1200, 1600, 2000, 2400, 2800, 3200, 3600, 4000, 4500, 32768]
    ...
    for max_tokens in max_token_values:
        ...
        out_file = f'{args.output_dir}/{model_name}/{args.data_name}/{out_file_prefix}_k{args.n_sampling}_s{args.start_idx}_e{args.end_idx}_token{max_tokens}.jsonl'
        ...
        if 'llm' in locals():
            ...
            time.sleep(5)  # 增加等待时间确保资源释放

        # 模型初始化
        # 明确可用的GPU数量，相应的进行并行处理
        ...
        # for each max_tokens value, generate completions
        file_outputs = []
        correct_cnt = 0
        ...
        results[max_tokens] = {
            ...
            'pass@k': sum(pass_at_k_list)/len(pass_at_k_list) if pass_at_k_list else None
        }         
        # 写文件
        ...
        with open(temp_out_file, 'w', encoding='utf-8') as f:
            ...
            for d in tqdm(file_outputs, "writing generation to jsonl file..."):
                # d['accuracy'] = correct_cnt / len(examples)
                # d['pass_at_k'] = sum(pass_at_k_list) / len(pass_at_k_list) if pass_at_k_list else 0
        ...
        gc.collect()
        torch.cuda.empty_cache()
        time.sleep(10)  # 增加显存释放缓冲时间
     # Print overall results
    print("\n\n=== 所有实验最终结果 ===")
    for max_token, res in results.items():
        print(f"max_tokens={max_token}:")
        print(f"  Accuracy: {res['accuracy']:.4f}")
        if res['pass@k'] is not None:
            print(f"  Pass@{args.k}: {res['pass@k']:.4f}")
        else:
            print(f"  Pass@1: {res['accuracy']:.4f}")
